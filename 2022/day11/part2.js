...
function calculateStressMediator() {
  return Object.keys(monkeyTracker)
    .map((monkey) => monkeyTracker[monkey].divisibleBy)
    .reduce((total, i) => total * i, 1);
}
...
function conductTestAndThrowToMonkey({
  ...
  const worryLevelAfterBored = input % stressMediator;
...
function main() {
  buildMonkeyTracker();
  stressMediator = calculateStressMediator();
